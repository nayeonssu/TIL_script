...
thumbNailA[0].addEventListener('click',function(){
    ...
    //클릭하고 싶은 이미지 넣고 싶을때
    popupBg.children[0].children[0].src = this.children[0].src//대입되는 위치
});
...
function popupShow() {
    return popupBg.style.display = 'flex';//flex와 block이 다르다. | flex로 디자인을 해서 다시 flex작성해주면 된다.(원래 가지고 있던 속성을 그대로 넣는것이다.)
}
//3. 배경 클릭 시 팝업 숨기기
