...
//input의 초기값 또는 입력한 이후의 값을 확인하는 속성으 value
//객체.속성
//사용자정의함수 없이 속성 및 내장함수를 작성하면 사용자 동작과 관계 없이 즉시 실행된다.
console.log(`사용자가 입력한 키 : ${userHeight.value}`);
console.log(`사용자가 입력한 몸무게 : ${userWeight.value}`);
//사용자가 키를 입력한 후 프로그램이 그 값을 인식하게 만드는 법
//값을 실시간으로 입력하는 동안은 프로그램 과부하 및 보안문제로 함수를 작동하게 해선 안되며 결정버튼을 따로 만들어두고 버튼에 특정동작을 해야만 함수가 실행되게 만들어야한다.
//실시간 함수 허용하는 경우) 보안이 필요하지 않는 가벼운 프로그래밍 부분 (예) 리뷰 글자 수 제한 등
...
function weightCal(){
    ...
    //사용자몸무게 - 적정체중 = 초과kg
    // result = (userHeight-100)*0.9
    //결과예) 당신의 적정체중은 ?kg이며 ?kg 초과되셨습니다.
    //const result = (userWeight.value-normalWeight);
