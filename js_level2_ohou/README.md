...
* thum4에 마우스를 올렸다 -> big이미지의 src속성값이 big4(값) 변경
* line 1 ~ 16 thumnails.js
...
* `body, html {scroll-behavior:smooth;}` 스크롤 애니메이션 추가(자연스럽게 내려가는 애니메이션)
## 7. 내일출발 클릭 시 배송정보 출력
...
* **select태그 비활성화 속성** : `DOM:disabled = true` -> 비활성화를 참으로 설정한다.
* **select태그 활성화 속성** : `DOM:disabled = false` -> 비활성화를 참으로 설정한다. DOM -> select태그가 된다.
...
## 18. 위 (17)번 진행한 상태에서 바로구매 클릭 시 비회원일 경우 로그인 새창 띄움, 회원일 경우 바로구매 페이지로 이동
