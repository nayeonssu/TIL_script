...
console.log(btn);
//반복문 없이 mouseover, mouseout 이벤트 작성법
...
//전체동의하기 클릭 시 나머지 필수양관들 모두 선택하기
//객체.checked -> 선택상태를 만들어주는 속성
const lawAllChk = document.querySelector('#all');//전체동의 1개
const lawChk = document.querySelectorAll('input[name^=law]');//개별선택
console.log(lawAllChk,lawChk);
lawAllChk.addEventListener('click',function(){
    console.log(this.checked)//활성화(true) 비활성화(false)
    //전체동의하기가 선택이 되면 나머지 다 선택이 되어라
    if(lawAllChk.checked == true){//현재(this)선택상태(checked)가 선택됐다면(true)
        for(let i of lawChk){
...
for(let i of tabContents){
    i.style.display = 'none'; //모든 내용 숨기기
}
...
tabTitle.forEach((obj,ind)=>{
    ...
    obj.addEventListener('click',()=>{
        //클릭한 대상과 동이ㄹ한 인덱스의 내용 보이기
        console.log(ind);
        for(let i of tabContents){
            i.style.display = 'none'; //모든 내용 숨기기
        }
        tabContents[ind].style.display = 'block'//제목을 클릭했을 때 다시 보이도록
    })
})